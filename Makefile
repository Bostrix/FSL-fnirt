...
# Additional LDFLAGS for libraries
ADDED_LDFLAGS = -L${HOME}/FSL-fnirt/znzlib -L${HOME}/FSL-fnirt/warpfns -L${HOME}/FSL-fnirt/meshclass -L${HOME}/FSL-fnirt/basisfield \
                -L${HOME}/FSL-fnirt/miscmaths -lfsl-znz -lfsl-warpfns -lfsl-meshclass -lfsl-basisfield -lfsl-miscmaths
...
fnirt: libraries ${FNIRT_OBJS} ${LIB_OBJS}
	${CXX} ${CXXFLAGS} -o $@ ${FNIRT_OBJS} ${LIB_OBJS} ${LDFLAGS} ${ADDED_LDFLAGS} -lblas -llapack -lz

invwarp: libraries ${INVWARP_OBJS} ${LIB_OBJS}
	${CXX} ${CXXFLAGS} -o $@ ${INVWARP_OBJS} ${LIB_OBJS} ${LDFLAGS} ${ADDED_LDFLAGS} -lblas -llapack -lz

mapintensities: libraries ${INTMAP_OBJS} ${LIB_OBJS}
	${CXX} ${CXXFLAGS} -o $@ ${INTMAP_OBJS} ${LIB_OBJS} ${LDFLAGS} ${ADDED_LDFLAGS} -lblas -llapack -lz

make_fnirt_field: libraries ${MAKE_FNIRT_FIELD_OBJS} ${LIB_OBJS}
	${CXX} ${CXXFLAGS} -o $@ ${MAKE_FNIRT_FIELD_OBJS} ${LIB_OBJS} ${LDFLAGS} ${ADDED_LDFLAGS} -lblas -llapack -lz
